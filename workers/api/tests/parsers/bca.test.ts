...
import { bcaParser } from '../../src/parsers/bca';
...
function makeEmail(overrides: Partial<EmailInput> = {}): EmailInput {
  return {
    messageId: 'msg-bca-001',
    subject: 'Notifikasi Transaksi',
    from: 'info@klikbca.com',
...
describe('bcaParser sender patterns', () => {
  it('matches info@klikbca.com', () => {
    const match = bcaParser.senderPatterns.some(p => p.test('info@klikbca.com'));
    expect(match).toBe(true);
  });

  it('matches noreply@bca.co.id', () => {
    const match = bcaParser.senderPatterns.some(p => p.test('noreply@bca.co.id'));
    expect(match).toBe(true);
  });

  it('matches notification@klikbca.com', () => {
    const match = bcaParser.senderPatterns.some(p => p.test('notification@klikbca.com'));
    expect(match).toBe(true);
  });

  it('does not match random@example.com', () => {
    const match = bcaParser.senderPatterns.some(p => p.test('random@example.com'));
    expect(match).toBe(false);
  });
});

// ─── findParserForEmail routing ──────────────────────────────────────
describe('findParserForEmail — BCA routing', () => {
  it('routes info@klikbca.com to BCA parser', () => {
    const parser = findParserForEmail('info@klikbca.com');
    expect(parser).not.toBeNull();
    expect(parser!.platform).toBe('bca');
  });

  it('routes alerts@bca.co.id to BCA parser', () => {
    const parser = findParserForEmail('alerts@bca.co.id');
    expect(parser).not.toBeNull();
    expect(parser!.platform).toBe('bca');
  });
});
...
describe('bcaParser — debit (expense)', () => {
  it('parses debit notification with correct amount and type', () => {
    ...
    expect(result!.platform).toBe('bca');
  ...
  it('detects expense with "pengeluaran" keyword', () => {
    const email = makeEmail({
      body: 'Pengeluaran sebesar Rp1.250.000 telah dicatat.',
    });
...
describe('bcaParser — credit (income)', () => {
...
// ─── Category Detection ─────────────────────────────────────────────
describe('bcaParser — category detection', () => {
  ...
  it('detects transfer category with "kirim" keyword', () => {
    const email = makeEmail({
      body: 'Kirim uang sebesar Rp100.000 ke rekening BNI.',
    });
...
describe('bcaParser — negative / edge cases', () => {
  ...
  it('handles amount with "nominal" prefix', () => {
    const email = makeEmail({
      body: 'Notifikasi debet. Nominal: Rp 750.000. Rekening 1234567890.',
    });
